...
export const Stories: React.FC = () => {
  ...
  const handleSearchByKeyPress = (
    ...
    [event.code, event.key].includes("Enter") ? getStories(1, inputName) : null;
            ...
            <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4 mb-5">
              {stories.map((story) => (
                <div key={story.title} className="col d-flex">
                  <StoryCard title={story.title} idroute={story.id} />
                </div>
              ))}
            </div>
            {pageCount > 1 && (
              <>
                <div className="mb-4">
                  <Pagination
                    ...
                    onPageChange={(event) =>
                      getStories(event.selected + 1, inputName)
                    }
                  />
                </div>
